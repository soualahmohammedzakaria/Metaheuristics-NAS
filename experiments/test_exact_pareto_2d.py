...
# Ensure imports work when running: python experiments/test_exact_pareto_2d.py
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run exact 2D Pareto search for one dataset-device context."
    )
    parser.add_argument(
        "--csv",
...
def main() -> None:
    ...
    evaluator = Evaluator(benchmark, dataset=args.dataset, device=device)
