...
class DvolverEvaluator:
    ...
    def query_benchmark(self, architecture_id) -> dict:
        ...
        # If FLOPs are unavailable from benchmark API, derive a monotonic proxy from latency.
    ...
    @staticmethod
    def _count_ops(architecture: dict, op_name: str) -> int:
        count = 0
        for cell_key in ("normal_cell", "reduction_cell"):
            cell = architecture.get(cell_key, {})
            for block in cell.get("blocks", []):
                _, o1, _, o2 = block
                if NASSearchSpace.OPS[o1] == op_name:
                    count += 1
                if NASSearchSpace.OPS[o2] == op_name:
                    count += 1
        return count

    def compute_flops(self, architecture: dict, N: int, F: int) -> int:
        # Lightweight analytic proxy FLOPs based on operation mix and fixed macro config.
    ...
    def evaluate(self, architecture: dict) -> tuple[float, float]:
        self.evaluations += 1

        # Optional benchmark route when a benchmark-queryable genotype is provided.
        if self.benchmark is not None:
            ...
            if benchmark_genotype is not None:
                try:
                    ...
                    # Non-existent benchmark architecture gets dominated quickly.
                    return 0.0, 0.0
